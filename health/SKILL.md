...
**多平台路径矩阵、识别步骤、检查命令** 见 **[`references/health-methods.md`](references/health-methods.md)**；**`scripts/collect-context.sh`** 会打印各平台常见标记文件是否存在，便于交叉判断。
...
使用 **Claude Code** 时，若团队另有「健康度收集」类脚本（如 `collect-data.sh`），可与本 skill 摘要结果对照；本仓库不内置该脚本。
...
| 与外部健康度脚本并用 | 以本仓库 `collect-context.sh` 输出为事实基线；外部脚本结论冲突时在报告里并列列出 |
