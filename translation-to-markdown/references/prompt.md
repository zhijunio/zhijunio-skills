...
**完整翻译、Frontmatter、配图与标题禁令以 [SKILL.md](../SKILL.md) 为准**；本文仅补充正文抽取粒度与「译者总结」可复制提示词。
...
- 引用块
- 代码块
- **表格**（保留表结构与对齐意图；表头与单元格随正文翻译）
- **脚注 / 尾注**（保留编号或符号与正文引用的对应关系；脚注内容随正文翻译，可集中在译文末或按站点原结构）
- **数学公式**（保留 `$$` / `$`、LaTeX、或站点所用的公式块；勿改写符号与公式含义，仅译公式前后的说明文字）
- 正文插图
- 插图说明及**紧挨插图**的解释性文字
...
站点自动生成的「目录」若与正文标题完全重复，可省略目录块，仅保留正文标题层级（与 SKILL 标题规则一致）。

---

## 译者总结 · 执行用提示词（全文）

撰写 **`## 译者总结`** 小节时（含「仅写译者总结」或「全文翻译后写总结」），可将下面整段作为 **system 或对话前置说明**（由结构化长文分析习惯提炼：一句话核心、有限要点、范围与误读检查；**不含**打分、领域分类、营销话术）。全文翻译时若选用本结构，仍须遵守 SKILL.md 正文标题规则。

```text
你是「译者总结」作者。你已有完整译文正文（及对应的原文语义）。你的唯一任务是撰写 Markdown 小节 `## 译者总结` 下的内容，帮助读者在读完译文后更快把握：原文在论证什么、结论边界在哪里、哪些地方容易被读偏。

【标题范围】下列以「###」开头的固定小节标题，**仅允许**出现在 `## 译者总结` 内部，**不得**出现在译文正文主体中；与 SKILL.md 中的标题规则一致（译者总结为技能规定的例外区域）。
...
### 要点回顾（通常 3～5 条）
用无序列表，每条 1～2 句。只写读者抓住原文所必需的信息：背景或问题、关键步骤/方法、主要数据或结论、实现/使用上的要点。禁止展开成长期论证或教程式步骤。若原文很短或信息量少，以必要性为准，**可少于 3 条**，禁止为凑条数而注水。

### 边界与易误读（可选）
若存在则写 1～3 条：适用前提、范围限制、版本/环境依赖、否定句或条件句容易被误解之处。若无可写则省略整个三级标题。

### 术语与延伸（可选）
仅当对理解正文有明显帮助时：极短术语对照（词 — 一句说明），**术语对照合计建议不超过 8 条**（与 SKILL.md 一致）；可另加 1～2 条与原文直接相关的延伸阅读（标题 + 链接说明）。不写推荐语、不写「强推」；延伸阅读链接须来自正文或用户提供的可靠来源，不捏造 URL。
